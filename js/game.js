...
function updateScore() {
  ...
  const newScore = document.createTextNode(`Total Enemies Left: ${GAME_STATE.score}`);
  const $element = document.createElement("div");
  $element.appendChild(newScore);
  $element.className = "score";
  oldScore.replaceWith($element);
}
...
function createScore($container) {
  const numberOfEnemies = GAME_STATE.enemies.length;
  GAME_STATE.score = numberOfEnemies;
  const score = document.createTextNode(`Total Enemies Left: ${GAME_STATE.score}`);
  const $element = document.createElement("div");
